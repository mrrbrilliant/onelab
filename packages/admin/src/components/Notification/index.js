...
import { ACTIONS } from "../../store/StoreAction";

// const NotificationFactory = ({ data }) => ({
//   id: uuid(),
//   show: true,
//   data,
// });
...
export default function Notification() {
  ...
  return (
    <div className="fixed top-0 right-0 p-4 flex flex-col justify-center">
      {store.notifications.length > 0 &&
        store.notifications
          .filter((n) => n.show)
          .map((data) => {
            return <Mapper key={uuid()} data={data} hide={hide} />;
          })}
    </div>
  );
}

const Mapper = ({ data, hide }) => {
  let d = jrpc.parse(JSON.stringify(data));
  let { type, payload } = d;

  if (type === "success") {
    return <SuccessNotification data={payload} hide={hide} />;
  }
  if (type === "error") {
    return <ErrorNotification data={payload} hide={hide} />;
  }
  return <NormalNotification data={payload} hide={hide} />;
};

const NormalNotification = ({ data, hide }) => {
  useEffect(() => {
    setTimeout(() => {
      hide({ id: data.id });
    }, 5000);
...
const ErrorNotification = ({ data, hide }) => {
  useEffect(() => {
    setTimeout(() => {
      hide({ id: data.id });
    }, 5000);
...
const WarningNotification = ({ data, hide }) => {
  useEffect(() => {
    setTimeout(() => {
      hide({ id: data.id });
    }, 5000);
...
const SuccessNotification = ({ data, hide }) => {
  useEffect(() => {
    setTimeout(() => {
      hide({ id: data.id });
    }, 5000);
