...
const getAllMemberByWorkspace = async ({ school_id }, context) => {
	return await MEMBER.find({ school_id: school_id });
};

const getMemberByID = async ({ id }, context) => {
	return await MEMBER.findById(id);
};

const addMember = async (args, context) => {
	let new_member = new MEMBER({ ...args });
	return await new_member.save();
};

const updateMember = async (args, context) => {
	let { id, ...update } = args;
	await MEMBER.findOneAndUpdate({ _id: id }, { ...update });
	return await MEMBER.findById(id);
};

const removeMember = async ({ id }, context) => {
